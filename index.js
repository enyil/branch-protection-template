...
mutation($repository: ID!, $pattern: String!, $requiresApprovingReviews: Boolean, $requiredApprovingReviewCount: Int, $isAdminEnforced: Boolean, $restrictsPushes: Boolean, $requiresLinearHistory: Boolean, $allowsForcePushes: Boolean, $allowsDeletions: Boolean, $requiresCommitSignatures: Boolean, $requiresCodeOwnerReviews: Boolean) {
...
module.exports = (app) => {
	...
	app.on("repository.created", async (context) => {
        ...
        try {
          const branchProtectionRuleResponse = await context.octokit.graphql(addBranchProtection, {
            ...
            requiresCommitSignatures: rules[i].requiresCommitSignatures,
            requiresCodeOwnerReviews: rules[i].requiresCodeOwnerReviews
          });

          console.log(`Branch rule added. Pattern ${rules[i].pattern}`);
          console.log(branchProtectionRuleResponse);
        } catch (error) {
          console.log(error);
        }
			}
			...
			for (let j = 0; j < templateRepositoryTeams.data.length; j++) {
				const { status, url } = await context.octokit.request("PUT /orgs/{org}/teams/{team_slug}/repos/{owner}/{repo}", {
        ...
        console.log(`Team added: ${templateRepositoryTeams.data[j].slug}`);
